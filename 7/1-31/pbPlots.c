...
_Bool
CropLineWithinBoundary(NumberReference *x1Ref, NumberReference *y1Ref, NumberReference *x2Ref, NumberReference *y2Ref,
...
void DrawFilledRectangleWithBorder(RGBABitmapImage *image, double x, double y, double w, double h, RGBA *borderColor,
                                   RGBA *fillColor) {
    if (h > 0.0 && w > 0.0) {
...
void DrawXLabelsForPriority(double p, double xMin, double oy, double xMax, double xPixelMin, double xPixelMax,
                            NumberReference *nextRectangle, RGBA *gridLabelColor, RGBABitmapImage *canvas,
                            double *xGridPositions, size_t xGridPositionsLength, StringArrayReference *xLabels,
                            NumberArrayReference *xLabelPriorities, Rectangle **occupied, size_t occupiedLength,
                            _Bool textOnBottom) {
...
void DrawYLabelsForPriority(double p, double yMin, double ox, double yMax, double yPixelMin, double yPixelMax,
                            NumberReference *nextRectangle, RGBA *gridLabelColor, RGBABitmapImage *canvas,
                            double *yGridPositions, size_t yGridPositionsLength, StringArrayReference *yLabels,
                            NumberArrayReference *yLabelPriorities, Rectangle **occupied, size_t occupiedLength,
                            _Bool textOnLeft) {
    _Bool overlap, currentOverlaps;
...
double *ComputeGridLinePositions(size_t *returnArrayLength, double cMin, double cMax, StringArrayReference *labels,
                                 NumberArrayReference *priorities) {
    double *positions;
    ...
    for (i = 0.0; i < pNum; i = i + 1.0) {
        ...
        if (mode == 4.0) {
            ...
            priority = 1.0;
            if (rem == 1.0 || rem == 2.0 || rem == 3.0 || rem == 4.0 || rem == 6.0 || rem == 7.0 || rem == 8.0 ||
                rem == 9.0) {
                priority = 2.0;
            }
        }
        ...
        labels->stringArray[(int) (i)]->string = CreateStringDecimalFromNumber(
                &labels->stringArray[(int) (i)]->stringLength, num);
    }
...
double MapXCoordinateAutoSettings(double x, RGBABitmapImage *image, double *xs, size_t xsLength) {
    return MapXCoordinate(x, GetMinimum(xs, xsLength), GetMaximum(xs, xsLength),
                          GetDefaultPaddingPercentage() * ImageWidth(image),
                          (1.0 - GetDefaultPaddingPercentage()) * ImageWidth(image));
}

double MapYCoordinateAutoSettings(double y, RGBABitmapImage *image, double *ys, size_t ysLength) {
    return MapYCoordinate(y, GetMinimum(ys, ysLength), GetMaximum(ys, ysLength),
                          GetDefaultPaddingPercentage() * ImageHeight(image),
                          (1.0 - GetDefaultPaddingPercentage()) * ImageHeight(image));
}
...
_Bool
DrawScatterPlot(RGBABitmapImageReference *canvasReference, double width, double height, double *xs, size_t xsLength,
                double *ys, size_t ysLength, StringReference *errorMessage) {
    ScatterPlotSettings *settings;
    _Bool success;
...
_Bool DrawScatterPlotFromSettings(RGBABitmapImageReference *canvasReference, ScatterPlotSettings *settings,
    ...
    if (success) {
        ...
        /* Draw title */
        DrawText(canvas, floor(settings->width / 2.0 - GetTextWidth(settings->title, settings->titleLength) / 2.0),
                 floor(yPadding / 3.0), settings->title, settings->titleLength, GetBlack());
        ...
        for (i = 1.0; i <= 5.0; i = i + 1.0) {
            ...
            DrawXLabelsForPriority(i, xMin, originYPixels, xMax, xPixelMin, xPixelMax, nextRectangle, gridLabelColor,
                                   canvas, xGridPositions, xGridPositionsLength, xLabels, xLabelPriorities, occupied,
                                   occupiedLength, textOnBottom);
        }
        ...
        for (i = 1.0; i <= 5.0; i = i + 1.0) {
            ...
            DrawYLabelsForPriority(i, yMin, originXPixels, yMax, yPixelMin, yPixelMax, nextRectangle, gridLabelColor,
                                   canvas, yGridPositions, yGridPositionsLength, yLabels, yLabelPriorities, occupied,
                                   occupiedLength, textOnLeft);
        }
        ...
        /* x origin line */
        if (originYInside) {
            DrawLine1px(canvas, Round(xPixelMin), Round(originYPixels), Round(xPixelMax), Round(originYPixels),
                        GetBlack());
        }

        /* y origin line */
        if (originXInside) {
            DrawLine1px(canvas, Round(originXPixels), Round(yPixelMin), Round(originXPixels), Round(yPixelMax),
                        GetBlack());
        }

        /* Draw origin axis titles. */
        DrawTextUpwards(canvas, 10.0,
                        floor(originTextYPixels - GetTextWidth(settings->yLabel, settings->yLabelLength) / 2.0),
                        settings->yLabel, settings->yLabelLength, GetBlack());
        DrawText(canvas, floor(originTextXPixels - GetTextWidth(settings->xLabel, settings->xLabelLength) / 2.0),
                 yPixelMax + axisLabelPadding, settings->xLabel, settings->xLabelLength, GetBlack());
                ...
                for (i = 0.0; i < xsLength; i = i + 1.0) {
                    ...
                    if (prevSet) {
                        ...
                        if (success) {
                            ...
                            if (aStringsEqual(sp->lineType, sp->lineTypeLength, strparam(L"solid")) &&
                                sp->lineThickness == 1.0) {
                                ...
                                DrawLineBresenhamsAlgorithmThickPatterned(canvas, pxPrev, pyPrev, px, py,
                                                                          sp->lineThickness, linePattern,
                                                                          linePatternLength, patternOffset, sp->color);
...
_Bool DrawBarPlot(RGBABitmapImageReference *canvasReference, double width, double height, double *ys, size_t ysLength,
                  StringReference *errorMessage) {
    BarPlotSettings *settings;
    _Bool success;
...
_Bool DrawBarPlotFromSettings(RGBABitmapImageReference *canvasReference, BarPlotSettings *settings,
    ...
    if (success) {
        ...
        /* Draw title */
        DrawText(canvas, floor(ImageWidth(canvas) / 2.0 - GetTextWidth(settings->title, settings->titleLength) / 2.0),
                 floor(yPadding / 3.0), settings->title, settings->titleLength, GetBlack());
        DrawTextUpwards(canvas, 10.0,
                        floor(ImageHeight(canvas) / 2.0 - GetTextWidth(settings->yLabel, settings->yLabelLength) / 2.0),
                        settings->yLabel, settings->yLabelLength, GetBlack());
        ...
        if (settings->autoBoundaries) {
            if (ss >= 1.0) {
                ...
                for (s = 0.0; s < ss; s = s + 1.0) {
                    yMax = fmax(yMax, GetMaximum(settings->barPlotSeries[(int) (s)]->ys,
                                                 settings->barPlotSeries[(int) (s)]->ysLength));
                    yMin = fmin(yMin, GetMinimum(settings->barPlotSeries[(int) (s)]->ys,
                                                 settings->barPlotSeries[(int) (s)]->ysLength));
                }
            } else {
                yMax = 10.0;
        ...
        for (i = 1.0; i <= 5.0; i = i + 1.0) {
            DrawYLabelsForPriority(i, yMin, xPixelMin, yMax, yPixelMin, yPixelMax, nextRectangle, gridLabelColor,
                                   canvas, yGridPositions, yGridPositionsLength, yLabels, yLabelPriorities, occupied,
                                   occupiedLength, true);
        }
        ...
        for (n = 0.0; n < bs; n = n + 1.0) {
            ...
            x = xPixelMin + (n + 0.5) * (ss * barWidth + (ss - 1.0) * barSeparation) + n * groupSeparation -
                textwidth / 2.0;
...
void DrawQuadraticBezierCurve(RGBABitmapImage *image, double x0, double y0, double cx, double cy, double x1, double y1,
                              RGBA *color) {
    double t, dt, dx, dy;
...
void
QuadraticBezierPoint(double x0, double y0, double cx, double cy, double x1, double y1, double t, NumberReference *x,
                     NumberReference *y) {
    x->numberValue = pow(1.0 - t, 2.0) * x0 + (1.0 - t) * 2.0 * t * cx + pow(t, 2.0) * x1;
    y->numberValue = pow(1.0 - t, 2.0) * y0 + (1.0 - t) * 2.0 * t * cy + pow(t, 2.0) * y1;
}

void DrawCubicBezierCurve(RGBABitmapImage *image, double x0, double y0, double c0x, double c0y, double c1x, double c1y,
                          double x1, double y1, RGBA *color) {
    double t, dt, dx, dy;
    NumberReference *xs, *ys, *xe, *ye;
...
void
CubicBezierPoint(double x0, double y0, double c0x, double c0y, double c1x, double c1y, double x1, double y1, double t,
                 NumberReference *x, NumberReference *y) {
    x->numberValue = pow(1.0 - t, 3.0) * x0 + pow(1.0 - t, 2.0) * 3.0 * t * c0x + (1.0 - t) * 3.0 * pow(t, 2.0) * c1x +
                     pow(t, 3.0) * x1;

    y->numberValue = pow(1.0 - t, 3.0) * y0 + pow(1.0 - t, 2.0) * 3.0 * t * c0y + (1.0 - t) * 3.0 * pow(t, 2.0) * c1y +
                     pow(t, 3.0) * y1;
}
...
void
BresenhamsCircleDrawingAlgorithm(RGBABitmapImage *canvas, double xCenter, double yCenter, double radius, RGBA *color) {
    double x, y, delta;
...
void
DrawFilledCircleMidpointAlgorithm(RGBABitmapImage *canvas, double xCenter, double yCenter, double radius, RGBA *color) {
    double d, x, y;
...
void
DrawFilledCircleBasicAlgorithm(RGBABitmapImage *canvas, double xCenter, double yCenter, double radius, RGBA *color) {
    double pixels, a, da, dx, dy;
...
void
DrawLineBresenhamsAlgorithmThick(RGBABitmapImage *canvas, double x1, double y1, double x2, double y2, double thickness,
                                 RGBA *color) {
...
void DrawLineBresenhamsAlgorithmThickPatterned(RGBABitmapImage *canvas, double x1, double y1, double x2, double y2,
                                               double thickness, _Bool *pattern, size_t patternLength,
                                               NumberReference *offset, RGBA *color) {
    double x, y, dx, dy, incX, incY, pdx, pdy, es, el, err, t, r;
...
wchar_t *
CreateStringScientificNotationDecimalFromNumberAllOptions(size_t *returnArrayLength, double decimal, _Bool complete) {
    ...
    result = AppendString(&resultLength, result, resultLength, mantissaReference->string,
                          mantissaReference->stringLength);
    if (complete) {
        additional = 16.0;
    ...
    result = AppendString(&resultLength, result, resultLength, exponentReference->string,
                          exponentReference->stringLength);
...
_Bool CreateNumberFromDecimalStringWithCheck(wchar_t *string, size_t stringLength, NumberReference *decimalReference,
                                             StringReference *errorMessage) {
...
_Bool
CreateNumberFromStringWithCheck(wchar_t *string, size_t stringLength, double base, NumberReference *numberReference,
                                StringReference *errorMessage) {
    _Bool success;
    ...
    if (base >= 2.0 && base <= 36.0) {
        success = ExtractPartsFromNumberString(string, stringLength, base, numberIsPositive, beforePoint, afterPoint,
                                               exponentIsPositive, exponent, errorMessage);

        if (success) {
            numberReference->numberValue = CreateNumberFromParts(base, numberIsPositive->booleanValue,
                                                                 beforePoint->numberArray,
                                                                 beforePoint->numberArrayLength,
                                                                 afterPoint->numberArray, afterPoint->numberArrayLength,
                                                                 exponentIsPositive->booleanValue,
                                                                 exponent->numberArray, exponent->numberArrayLength);
        }
    } else {
        success = false;
...
double CreateNumberFromParts(double base, _Bool numberIsPositive, double *beforePoint, size_t beforePointLength,
                             double *afterPoint, size_t afterPointLength, _Bool exponentIsPositive, double *exponent,
...
_Bool ExtractPartsFromNumberString(wchar_t *n, size_t nLength, double base, BooleanReference *numberIsPositive,
                                   NumberArrayReference *beforePoint, NumberArrayReference *afterPoint,
                                   BooleanReference *exponentIsPositive, NumberArrayReference *exponent,
    ...
    if (i < nLength) {
        ...
        success = ExtractPartsFromNumberStringFromSign(n, nLength, base, i, beforePoint, afterPoint, exponentIsPositive,
                                                       exponent, errorMessages);
    } else {
        success = false;
...
_Bool ExtractPartsFromNumberStringFromSign(wchar_t *n, size_t nLength, double base, double i,
                                           NumberArrayReference *beforePoint, NumberArrayReference *afterPoint,
                                           BooleanReference *exponentIsPositive, NumberArrayReference *exponent,
                                           StringReference *errorMessages) {
    _Bool success, done;
    ...
    if (count >= 1.0) {
        ...
        if (i < nLength) {
            success = ExtractPartsFromNumberStringFromPointOrExponent(n, nLength, base, i, afterPoint,
                                                                      exponentIsPositive, exponent, errorMessages);
        } else {
...
_Bool ExtractPartsFromNumberStringFromPointOrExponent(wchar_t *n, size_t nLength, double base, double i,
                                                      NumberArrayReference *afterPoint,
                                                      BooleanReference *exponentIsPositive,
                                                      NumberArrayReference *exponent, StringReference *errorMessages) {
    _Bool success, done;
    double count, j;
    ...
    if (n[(int) (i)] == '.') {
        ...
        if (i < nLength) {
            ...
            if (count >= 1.0) {
                ...
                if (i < nLength) {
                    success = ExtractPartsFromNumberStringFromExponent(n, nLength, base, i, exponentIsPositive,
                                                                       exponent, errorMessages);
                } else {
                    exponent->numberArray = (double *) malloc(sizeof(double) * (0.0));
                    exponent->numberArrayLength = 0.0;
        ...
        if (i < nLength) {
            success = ExtractPartsFromNumberStringFromExponent(n, nLength, base, i, exponentIsPositive, exponent,
                                                               errorMessages);
            afterPoint->numberArray = (double *) malloc(sizeof(double) * (0.0));
            afterPoint->numberArrayLength = 0.0;
        } else {
...
_Bool ExtractPartsFromNumberStringFromExponent(wchar_t *n, size_t nLength, double base, double i,
                                               BooleanReference *exponentIsPositive, NumberArrayReference *exponent,
                                               StringReference *errorMessages) {
    _Bool success, done;
    double count, j;
...
_Bool StringToNumberArrayWithCheck(wchar_t *str, size_t strLength, NumberArrayReference *numberArrayReference,
                                   StringReference *errorMessage) {
    ...
    for (i = 0.0; i < numberStringsLength; i = i + 1.0) {
        ...
        success = CreateNumberFromDecimalStringWithCheck(trimmedNumberString, trimmedNumberStringLength,
                                                         numberReference, errorMessage);
...
double Error(double x) {
    ...
    if (x == 0.0) {
        ...
        tau = t * exp(-pow(x, 2.0) + c1 +
                      t * (c2 + t * (c3 + t * (c4 + t * (c5 + t * (c6 + t * (c7 + t * (c8 + t * (c9 + t * c10)))))))));
...
wchar_t *GetPixelFontData(size_t *returnArrayLength) {
    static wchar_t monster[] = L" monster string literal here ";
...
void AssertStringArraysEqual(StringReference **a, size_t aLength, StringReference **b, size_t bLength,
                             NumberReference *failures) {
    double i;

    if (aLength == bLength) {
        for (i = 0.0; i < aLength; i = i + 1.0) {
            AssertStringEquals(a[(int) (i)]->string, a[(int) (i)]->stringLength, b[(int) (i)]->string,
                               b[(int) (i)]->stringLength, failures);
        }
    } else {
        failures->numberValue = failures->numberValue + 1.0;
    }
...
double *ConvertToPNGWithOptions(size_t *returnArrayLength, RGBABitmapImage *image, double colorType, _Bool setPhys,
                                double pixelsPerMeter, double compressionLevel) {
    PNGImage *png;
...
double *PNGSerializeChunks(size_t *returnArrayLength, PNGImage *png) {
    ...
    Write4BytesBE(data, dataLength,
                  CRC32OfInterval(data, dataLength, position->numberValue - chunkLength - 4.0, chunkLength + 4.0),
                  position);
                    ...
                    if (ihdr->CompressionMethod == 0.0) {
                        if (ihdr->FilterMethod == 0.0) {
                            if (ihdr->InterlaceMethod == 0.0) {
                                ...
                                success = false;
                                errorMessages->string = AppendString(&errorMessages->stringLength,
                                                                     errorMessages->string, errorMessages->stringLength,
                                                                     strparam(L"Interlace method not supported."));
                            }
                        } else {
                            success = false;
                            errorMessages->string = AppendString(&errorMessages->stringLength, errorMessages->string,
                                                                 errorMessages->stringLength,
                                                                 strparam(L"Filter method not supported."));
                        }
                    } else {
                        success = false;
                        errorMessages->string = AppendString(&errorMessages->stringLength, errorMessages->string,
                                                             errorMessages->stringLength,
                                                             strparam(L"Compression type not supported."));
                    }
                } else {
                    success = false;
                    errorMessages->string = AppendString(&errorMessages->stringLength, errorMessages->string,
                                                         errorMessages->stringLength,
                                                         strparam(L"Bit depth not supported."));
                }
            } else {
                success = false;
                errorMessages->string = AppendString(&errorMessages->stringLength, errorMessages->string,
                                                     errorMessages->stringLength,
                                                     strparam(L"Color type not supported."));
            }
        }
    }
...
void
WriteStringToStingStream(wchar_t *stream, size_t streamLength, NumberReference *index, wchar_t *src, size_t srcLength) {
    double i;
...
_Bool
SubstringWithCheck(wchar_t *string, size_t stringLength, double from, double to, StringReference *stringReference) {
    _Bool success;
...
_Bool
SubstringEqualsWithCheck(wchar_t *string, size_t stringLength, double from, wchar_t *substring, size_t substringLength,
...
_Bool IndexOfString(wchar_t *string, size_t stringLength, wchar_t *substring, size_t substringLength,
                    NumberReference *indexReference) {
    double i;
    _Bool found;
...
_Bool ContainsString(wchar_t *string, size_t stringLength, wchar_t *substring, size_t substringLength) {
    return IndexOfString(string, stringLength, substring, substringLength,
                         (NumberReference *) malloc(sizeof(NumberReference)));
}
...
wchar_t *ReplaceString(size_t *returnArrayLength, wchar_t *string, size_t stringLength, wchar_t *toReplace,
                       size_t toReplaceLength, wchar_t *replaceWith, size_t replaceWithLength) {
    wchar_t *result;
    size_t resultLength;
    double i;
    BooleanReference *equalsReference;
...
wchar_t *ReplaceCharacter(size_t *returnArrayLength, wchar_t *string, size_t stringLength, wchar_t toReplace,
                          wchar_t replaceWith) {
    wchar_t *result;
    size_t resultLength;
    double i;
...
wchar_t *Trim(size_t *returnArrayLength, wchar_t *string, size_t stringLength) {
    ...
    if (lastWhitespaceLocationStart < lastWhitespaceLocationEnd) {
        result = Substring(&resultLength, string, stringLength, lastWhitespaceLocationStart + 1.0,
                           lastWhitespaceLocationEnd);
    } else {
        result = (wchar_t *) malloc(sizeof(wchar_t) * (0.0));
        resultLength = 0.0;
    }
...
StringReference **SplitByString(size_t *returnArrayLength, wchar_t *toSplit, size_t toSplitLength, wchar_t *splitBy,
                                size_t splitByLength) {
    StringReference **split;
    size_t splitLength;
    wchar_t *next;
    size_t nextLength;
...
double *
ReadXbytes(size_t *returnArrayLength, double *data, size_t dataLength, NumberReference *position, double length) {
    double *r;
    size_t rLength;
    double i;
...
ZLIBStruct *ZLibCompressNoCompression(double *data, size_t dataLength) {
    ...
    zlibStruct->FLG = 1.0;
    zlibStruct->CompressedDataBlocks = DeflateDataNoCompression(&zlibStruct->CompressedDataBlocksLength, data,
                                                                dataLength);
...
ZLIBStruct *ZLibCompressStaticHuffman(double *data, size_t dataLength, double level) {
    ...
    zlibStruct->CMF = 120.0;
    zlibStruct->FLG = 1.0;
    zlibStruct->CompressedDataBlocks = DeflateDataStaticHuffman(&zlibStruct->CompressedDataBlocksLength, data,
                                                                dataLength, level);
    zlibStruct->Adler32CheckValue = ComputeAdler32(data, dataLength);
...
double And4Byte(double n1, double n2) {
    if ((double) n1 >= 0.0 && (double) n1 <= (double) 0xFFFFFFFFUL && (double) n2 >= 0.0 &&
        (double) n2 <= (double) 0xFFFFFFFFUL) {
...
double And2Byte(double n1, double n2) {
    if ((double) n1 >= 0.0 && (double) n1 <= (double) 0xFFFFUL && (double) n2 >= 0.0 &&
        (double) n2 <= (double) 0xFFFFUL) {
        return (unsigned long) n1 & (unsigned long) n2;
    } else {
        return 0.0;
    }
...
double ShiftLeft4Byte(double b, double amount) {
    if ((double) b >= 0.0 && (double) b <= (double) 0xFFFFFFFF && (double) amount >= 0.0 &&
        (double) amount < (double) 32) {
...
double *DeflateDataStaticHuffman(size_t *returnArrayLength, double *data, size_t dataLength, double level) {
    ...
    for (i = 0.0; i < dataLength;) {
        ...
        if (match->booleanValue) {
            ...
            GetDeflateDistanceCode(distanceReference->numberValue, distanceCode, distanceAdditionReference,
                                   distanceAdditionLengthReference, bitReverseLookupTable, bitReverseLookupTableLength);
        }

        if (!match->booleanValue) {
            GetDeflateStaticHuffmanCode(data[(int) (i)], code, length, bitReverseLookupTable,
                                        bitReverseLookupTableLength);
            ...
            GetDeflateStaticHuffmanCode(compressedCode->numberValue, code, length, bitReverseLookupTable,
                                        bitReverseLookupTableLength);
            AppendBitsToBytesRight(bytes, bytesLength, currentBit, code->numberValue, length->numberValue);
            AppendBitsToBytesRight(bytes, bytesLength, currentBit, lengthAddition->numberValue,
                                   lengthAdditionLength->numberValue);
            AppendBitsToBytesRight(bytes, bytesLength, currentBit, distanceCode->numberValue, 5.0);
            AppendBitsToBytesRight(bytes, bytesLength, currentBit, distanceAdditionReference->numberValue,
                                   distanceAdditionLengthReference->numberValue);
...
void FindMatch(double *data, size_t dataLength, double pos, NumberReference *distanceReference,
               NumberReference *lengthReference, BooleanReference *match, double level) {
    double i, j;
...
void
GetDeflateStaticHuffmanCode(double b, NumberReference *code, NumberReference *length, double *bitReverseLookupTable,
                            size_t bitReverseLookupTableLength) {
    double reversed;
...
void GetDeflateLengthCode(double length, NumberReference *code, NumberReference *lengthAddition,
                          NumberReference *lengthAdditionLength) {
...
void GetDeflateDistanceCode(double distance, NumberReference *code, NumberReference *distanceAdditionReference,
                            NumberReference *distanceAdditionLengthReference, double *bitReverseLookupTable,
                            size_t bitReverseLookupTableLength) {
    double reversed;
