...
void Graph::generate(int size, double density) {
    ...
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            ...
            else if (i > j) adjMatrix[i][j] = adjMatrix[j][i];
...
void Graph::inputFromFile() {
    ...
    for (int i = 0; i < graphSize; i++) {
        for (int j = 0; j < graphSize; j++) f >> adjMatrix[i][j];
    }
...
void Graph::clear() {
    for (int i = 0; i < graphSize; i++) adjMatrix[i].clear();
    adjMatrix.clear();
}
...
void Graph::generateAnswer(int size, double density, bool print) {
    ...
    for (int i = 0; i < size - 1; i++) {
        ...
        if (i > 0) answer[chain[i + 1]] = adjMatrix[chain[i]][chain[i + 1]] + answer[chain[i]];
        else answer[chain[i + 1]] = adjMatrix[chain[i]][chain[i + 1]];
